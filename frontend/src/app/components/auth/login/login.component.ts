    ...
    if(this.loginForm.valid && this.loginForm.value.email && this.loginForm.value.password) {
      this.AuthService.login({
        ...
        next: (data: LoginResponseType | DefaultResponseType) => {
          ...
          this.BasketService.getBasketCount().subscribe((count: {count: number} | DefaultResponseType) => {
            if((count as DefaultResponseType).error !== undefined) {
              throw new Error((count as DefaultResponseType).message);
            }
            const countValue = count as {count: number};
            if(countValue) {
              this.BasketService.count$.next(countValue.count);
            }
          })
        },
